...
time    child pro         par pro
 |                          end         
 |       init will adopt child pro if par end before child pro, damon process                   
 |        end                  
...
if a program which has a socket fd opend, in server it's the return value
of "accept", in client it's the connect(socket_fd,...)
if the program terminated without calling close or shutdown, the system
will help to do that.

when grab a tcp packet named FIN, it's no necessarily that the program
call close or shutdown, in fact, even if the programe run into segmentation
fault, the system will help to close the connection.

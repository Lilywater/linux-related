...
------------------------
virtual memory is the overal memory a process can use.And for a program, the stack size
is allocated statically when process is loading, and the heap size is the dynamically.

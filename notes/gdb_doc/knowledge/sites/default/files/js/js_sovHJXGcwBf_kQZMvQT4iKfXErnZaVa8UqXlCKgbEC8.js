ajq(document).ready(function () {
		...
		*/
		...
		console.log(ajq('#edit-language_chzn').width());
...
function insertMarkdownInsteadOfHtml() {
	// 15min hack... sorry if this kerplodes
	// added check to skip markdown insert for page content type.
	if (!ajq('form').hasClass('node-page-form') && ajq('#edit-field-kcs-img-files-und-ajax-wrapper').length !== 0) {
...
function removeMarkdownUI() {
	...
	var page_type = ajq('#page-node-form .form-type-select select');
	...
	if (nid != null) {
		ajq.getJSON(URL + '?jsoncallback=?', {
			...
			if (data.system_id_array.length > 0) {
				...
				content += '</ul>';
				sideBar.append('<div id="case-links" class="block"><h2 class="block-title">Case links <sup>BETA</sup></h2><div class="content">'+content+'</div></div>');
			}
		});
	}
}
